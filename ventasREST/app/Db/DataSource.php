...
class DataSource
{
	...
	public function __construct(){
		try{
			$this->cadenaConexion = "mysql:host=localhost;dbname=millonaria_cultura";
			$this->conexion = new PDO($this->cadenaConexion,"root","onepieceace");
	...
	public function ejecutarConsulta($sql = "",$values = array()){
        ...
        public function execConsultaObject($sql = "",$values = array(), $clase){
		if($sql != ""){
			$consulta = $this->conexion->prepare($sql);
			$consulta->execute($values);
			$tabla_datos = $consulta->fetchAll(PDO::FETCH_CLASS,"app\\Model\\".$clase);
